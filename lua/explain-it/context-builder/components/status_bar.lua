...
--- @class explain-it.context-builder.components.status_bar.StatusBarProps
--- @field state table

--- Status bar component showing current state
---@param ctx morph.Ctx<explain-it.context-builder.components.status_bar.StatusBarProps, any>
function M.StatusBar(ctx)
  ...
  if file_count > 0 or snippet_count > 0 then
    ...
    if file_count > 0 and snippet_count > 0 then table.insert(status_items, ", ") end
  ...
  -- Keybind hints
  table.insert(status_items, " | ")
  table.insert(status_items, h.Comment({}, "[f: files, i: instruction, e: export, ?: help]"))
